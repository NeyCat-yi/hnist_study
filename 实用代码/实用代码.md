...
| **字母** | **代表含义 (英文)** | **说明 (中文)**                                     |
| -------- | ------------------- | --------------------------------------------------- |
| **C**    | **Context**         | **上下文**：提供任务的背景信息。                    |
| **O**    | **Objective**       | **目标**：明确希望 AI 完成的具体任务。              |
| **S**    | **Style**           | **风格**：指定写作风格（如：像鲁迅一样写作）。      |
| **T**    | **Tone**            | **语气**：设定回复的情感基调（如：专业、幽默）。    |
| **A**    | **Audience**        | **受众**：明确回答是给谁看的（如：5岁小孩或专家）。 |
| **R**    | **Response**        | **响应格式**：规定输出格式（如：表格、Markdown）。  |
...
git status               # 看一下
git add .                # 存一下
git commit -m "备注信息"  # 包一下
git pull origin main     # 对一下 (防止冲突)
git push origin main     # 传一下
...
```bash
# -u 参数只需第一次使用，以后直接 git push 即可
git push -u origin main
```

## 常见检查命令

如果你不确定目前连接到了哪个库，可以使用以下命令排查：

- **查看连接地址**：`git remote -v`（确认是否指向了正确的 `GoldFlotation/Flotation-Webfront`）。
- **查看当前分支**：`git branch`（确认你在 `main` 而不是 `master`）。
...
- **`feat` (feature)**: **新功能**。你往项目里添加了以前没有的东西。
  - *例子：* `feat: 添加用户登录验证功能`
- **`fix`**: **修 Bug**。修复了程序中的错误或漏洞。
  - *例子：* `fix: 修复首页图片无法加载的问题`
- **`docs`**: **文档相关**。只改动了文档（如 README.md），没动代码逻辑。
  - *例子：* `docs: 更新项目安装指南`

| **标签**       | **含义** | **适用场景**                                        |
| -------------- | -------- | --------------------------------------------------- |
| **`style`**    | 格式     | 不影响代码含义的改动（空格、格式化、缺少分号等）。  |
| **`refactor`** | 重构     | 既不是修 Bug 也不是加功能，而是为了让代码更整洁。   |
| **`perf`**     | 性能     | 提高性能的代码更改。                                |
| **`test`**     | 测试     | 添加缺失的测试或更正现有的测试。                    |
| **`chore`**    | 杂务     | 构建过程或辅助工具的变动（比如修改 `.gitignore`）。 |